#!/usr/bin/env bash
# Wipe parity: bounded (400) and unbounded-to-completion dumps vs the Rust
# oracle, at seeds 42 and 1337, for every wipe-* line in cases.txt.
...
if [ ! -x "$BIN" ]; then
  echo "wipe.sh: $BIN is missing. Run bin/build first." >&2
  exit 1
fi
...
while IFS='|' read -r name input args; do
  ...
  case "$name" in wipe-*) ;; *) continue ;; esac
...
echo "wipe parity: $pass passed, $fail failed"
if [ "$fail" -gt 0 ]; then
  printf 'FAILED: %s\n' "${failed[@]}"
  exit 1
fi
